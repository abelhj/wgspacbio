...
time seq 0 $((EXAMPLES_THREADS - 1)) | parallel -j ${EXAMPLES_THREADS} -q --halt 2 --line-buffer /opt/deepvariant/bin/make_examples_somatic \
    ...
    --vsc_max_fraction_snps_for_non_target_sample "0.5" --vsc_min_count_snps "1" --vsc_min_fraction_indels "0.1" --vsc_min_fraction_snps "0.02" --task {}
...
#concatenate vcf
bcftools concat -a ${VCF_OUT}/*.vcf.gz | bcftools sort | bgzip > ${TUMOR_SAMPLE}.ds.lumos.merged.vcf.gz
tabix -p vcf ${TUMOR_SAMPLE}.ds.lumos.merged.vcf.gz
